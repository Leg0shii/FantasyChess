...
from api.models import JoinGameRequest, LeaveGameRequest
...
async def create_game(
    ...
    return {"message": "Game created", "session_key": game.id}

@router.post("/join")
async def join_game(request: JoinGameRequest) -> dict[str, str]:
    """Join an existing game session."""
    game = get_game_manager().get_game(request.session_key)
    if len(game.players) == 1:
        game.players.append(Player(ColorType.BLACK, request.player_name))
        return {"message": "Joined game", "session_key": game.id}
    else:
...
async def leave_game(request: LeaveGameRequest) -> dict[str, str]:
    ...
    return {"message": "Left game", "session_key": game.id}
